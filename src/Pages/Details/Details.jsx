...
const Details = () => {
  ...
  try {
    const response = await axios.post(`${apiUrl}/api/bookticket`, ticketData);
    console.log('Ticket created successfully:', response.data);
  } catch (error) {
    console.error('Error creating ticket:', error);
  }
        Navigate('/bookedtickets', { state: { formData, bus } })
    };
                    ...
                    <div className='col-md-8 shadow p-3 mb-5 bg-light p-4 rounded mt-3'>
                        <form className="row p-5" onSubmit={handleSubmit}>
                            ...
                            <div className="col-md-12 text-center">
                                <button type="submit" className={` btn btn-lg mt-3 `} style={{ color: "white", backgroundColor: "#198754" }} >
                                    Proceed To Payment
                                </button>
                            </div>
                        </form>
